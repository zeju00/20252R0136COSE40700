...
def analyze_experiment(exp_path, dir_name):
    ...
    for prefix, ts_data in [("client", client_ts_data), ("runner", runner_ts_data)]:
        ...
        combined_results[f'{prefix}_forward_avg_summed_power_w'] = calculate_average_power_in_intervals(summed_power_df, forward_intervals)
        combined_results[f'{prefix}_backward_avg_summed_power_w'] = calculate_average_power_in_intervals(summed_power_df, backward_intervals)
        ...
        if prefix == 'client' and summed_power_df is not None:
            ...
            if num_iters > 0:
                ...
                # --- 👇 [수정 1] Fwd, Comm, Bwd 모든 구간의 데이터 포인트 수 계산 ---
                fwd_points_df = summed_power_df[
                    (summed_power_df['timestamp'] >= rep_fwd_interval[0]) & 
                    (summed_power_df['timestamp'] <= rep_fwd_interval[1])
                ]
                comm_points_df = summed_power_df[
                    (summed_power_df['timestamp'] >= rep_comm_interval[0]) & 
                    (summed_power_df['timestamp'] <= rep_comm_interval[1])
                ]
                bwd_points_df = summed_power_df[
                    (summed_power_df['timestamp'] >= rep_bwd_interval[0]) & 
                    (summed_power_df['timestamp'] <= rep_bwd_interval[1])
                ]
                
                combined_results[f'{prefix}_rep_iter_fwd_points'] = len(fwd_points_df)
                combined_results[f'{prefix}_rep_iter_comm_points'] = len(comm_points_df)
                combined_results[f'{prefix}_rep_iter_bwd_points'] = len(bwd_points_df)
                # --- 👆 [수정 1] ---
...
def process_bandwidth_dir(dir_path, top_level_dir):
    ...
    # --- 👇 [수정 2] rep_iter_... 컬럼들을 client_cols 리스트에 추가 ---
    rep_iter_cols = [
        'rep_iter_fwd_points',
        'rep_iter_comm_points', # comm 포인트 수 다시 추가
        'rep_iter_bwd_points'
    ]
    ...
    client_cols = ['client_energy_efficiency'] + \
                  ...
                  [f'client_{col}' for col in rep_iter_cols] # rep_iter_cols 추가
    ...
    # --- 👇 [수정 3] 요약 CSV 컬럼 리스트에 comm 포인트 수 다시 추가 ---
    power_summary_cols = [
        'split_index',
        ...
        'client_rep_iter_comm_points', # comm 포인트 수 다시 추가
